#!/usr/bin/env python3
"""
Static part-based actor viewer.

Controls: C next model | ESC quit
...
class Golem:
    ...
    def draw(self, surf, sprite_set):
        ...
        for name in self.draw_order:
            wx, wy, lx, ly = pivots[name]
            blit_part(actor_surf, sprite_set[name], 0.0, wx + off_x, wy + off_y, lx, ly)
...
def draw_hud(surf, mode, has_glow):
    ...
    title = hf.render(f"{title_str}  -  Static Part-Based Actor", True, title_color)
...
def main():
    ...
    while True:
        ...
        draw_hud(screen, golem.mode, "fire" in golem.mode)
        pygame.display.flip()
